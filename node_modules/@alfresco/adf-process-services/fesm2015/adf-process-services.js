...
import { AlfrescoApiService, AppsProcessService, TranslationService, CustomEmptyContentTemplateDirective, CommentProcessService, ContentService, EmptyListComponent, ThumbnailService, LogService, UserPreferencesService, UserPreferenceValues, FormFieldModel, FormModel, PeopleProcessService, UserProcessModel, AuthenticationService, CardViewUpdateService, FormRenderingService, DataTableSchema, CustomLoadingContentTemplateDirective, AppConfigService, PaginationComponent, CardViewDateItemModel, CardViewTextItemModel, FormService, DataColumnListComponent, UploadService, TRANSLATION_PROVIDER, CoreModule, ProcessContentService, MOMENT_DATE_FORMATS, MomentDateAdapter, CardViewMapItemModel, BpmUserService, AppConfigValues, ActivitiContentService, UploadWidgetComponent, WidgetComponent, NodesApiService, SitesService, SearchService, ExternalAlfrescoApiService } from '@alfresco/adf-core';
...
class ProcessFilterService {
    ...
    getProcessFilters(appId) {
        return from(this.callApiProcessFilters(appId))
            ...
            /** @type {?} */
            let filters = [];
            response.data.forEach((filter$$1) => {
                /** @type {?} */
                let filterModel = new FilterProcessRepresentationModel(filter$$1);
    ...
    getProcessFilterById(filterId, appId) {
        return from(this.callApiProcessFilters(appId))
            .pipe(map((response) => {
            return response.data.find((filter$$1) => filter$$1.id === filterId);
        }), catchError((err) => this.handleProcessError(err)));
    }
    /**
    ...
    getProcessFilterByName(filterName, appId) {
        return from(this.callApiProcessFilters(appId))
            .pipe(map((response) => {
            return response.data.find((filter$$1) => filter$$1.name === filterName);
        }), catchError((err) => this.handleProcessError(err)));
    }
    /**
...
class TaskListComponent extends DataTableSchema {
    ...
    optimizeTaskDetails(instances) {
        instances = instances.map((task) => {
            ...
            if (task.created) {
                task.created = moment$1(task.created).format(this.FORMAT_DATE);
            }
            if (task.dueDate) {
                task.dueDate = moment$1(task.dueDate).format(this.FORMAT_DATE);
            }
            if (task.endDate) {
                task.endDate = moment$1(task.endDate).format(this.FORMAT_DATE);
            }
            return task;
        });
        return instances;
...
class TaskFilterService {
    ...
    getTaskFilterById(filterId, appId) {
        return from(this.callApiTaskFilters(appId)).pipe(map((response) => response.data.find((filter$$1) => filter$$1.id === filterId)), catchError((err) => this.handleError(err)));
    }
    /**
    ...
    getTaskFilterByName(taskName, appId) {
        return from(this.callApiTaskFilters(appId)).pipe(map((response) => response.data.find((filter$$1) => filter$$1.name === taskName)), catchError((err) => this.handleError(err)));
    }
    /**
...
class TaskFiltersComponent {
    ...
    selectFilter(newFilter) {
        if (newFilter) {
            this.currentFilter = this.filters.find((filter$$1, index) => newFilter.index === index ||
    ...
    selectFilterWithTask(taskId) {
        ...
        this.taskListService.getFilterForTaskById(taskId, this.filters).subscribe((filter$$1) => {
...
class AppsListComponent {
    ...
    filterApps(apps) {
        ...
        if (this.filtersAppId) {
            apps.filter((app) => {
                this.filtersAppId.forEach((filter$$1) => {
