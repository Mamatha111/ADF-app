...
import { map, catchError, switchMap, flatMap, filter, mergeMap, share, defaultIfEmpty, debounceTime } from 'rxjs/operators';
import { MatDialog, MAT_DIALOG_DATA, MAT_LABEL_GLOBAL_OPTIONS, MatAutocompleteModule, MatButtonModule, MatCardModule, MatCheckboxModule, MatChipsModule, MatDatepickerModule, MatDialogModule, MatGridListModule, MatIconModule, MatInputModule, MatListModule, MatNativeDateModule, MatOptionModule, MatProgressSpinnerModule, MatRadioModule, MatRippleModule, MatSelectModule, MatSlideToggleModule, MatTableModule, MatTabsModule, MatTooltipModule, MatMenuModule, MatAutocompleteTrigger, MatDialogRef } from '@angular/material';
import { FlexLayoutModule } from '@angular/flex-layout';
import { __values, __extends, __spread, __read } from 'tslib';
import { CommonModule, DatePipe } from '@angular/common';
import { Injectable, NgModule, Component, EventEmitter, Input, Output, ViewChild, ContentChild, Directive, TemplateRef, ViewEncapsulation, Inject, NgZone, defineInjectable, inject } from '@angular/core';
import { FormBuilder, Validators, FormControl, FormsModule, ReactiveFormsModule } from '@angular/forms';
import { AlfrescoApiService, AppsProcessService, ContentService, ThumbnailService, EmptyListComponent, CommentProcessService, TranslationService, CustomEmptyContentTemplateDirective, LogService, UserPreferencesService, UserPreferenceValues, FormFieldModel, FormModel, PeopleProcessService, UserProcessModel, AuthenticationService, CardViewUpdateService, FormRenderingService, AppConfigService, CardViewDateItemModel, CardViewTextItemModel, FormService, DataColumnListComponent, TRANSLATION_PROVIDER, CoreModule, ProcessContentService, PaginationComponent, CustomLoadingContentTemplateDirective, DataTableSchema, AppConfigValues, ActivitiContentService, MOMENT_DATE_FORMATS, MomentDateAdapter, CardViewMapItemModel, BpmUserService, UploadService, UploadWidgetComponent, NodesApiService, WidgetComponent, SitesService, SearchService, ExternalAlfrescoApiService } from '@alfresco/adf-core';
...
var ProcessFilterService = /** @class */ (function () {
    ...
    function (appId) {
        ...
        return from(this.callApiProcessFilters(appId))
            ...
            /** @type {?} */
            var filters = [];
            response.data.forEach(function (filter$$1) {
                /** @type {?} */
    ...
    function (filterId, appId) {
        ...
        return from(this.callApiProcessFilters(appId))
            .pipe(map(function (response) {
            return response.data.find(function (filter$$1) { return filter$$1.id === filterId; });
        }), catchError(function (err) { return _this.handleProcessError(err); }));
    };
    /**
    ...
    function (filterName, appId) {
        ...
        return from(this.callApiProcessFilters(appId))
            .pipe(map(function (response) {
            return response.data.find(function (filter$$1) { return filter$$1.name === filterName; });
        }), catchError(function (err) { return _this.handleProcessError(err); }));
    };
    /**
    ...
    function (appId) {
        ...
        return new Observable(function (observer) {
            forkJoin(runningObservable, completedObservable, allObservable).subscribe(function (res) {
                /** @type {?} */
                var filters = [];
                res.forEach(function (filter$$1) {
...
var TaskListComponent = /** @class */ (function (_super) {
    ...
    function (instances) {
        ...
        instances = instances.map(function (task) {
            ...
            if (task.created) {
                task.created = moment$1(task.created).format(_this.FORMAT_DATE);
            }
            if (task.dueDate) {
                task.dueDate = moment$1(task.dueDate).format(_this.FORMAT_DATE);
            }
            if (task.endDate) {
                task.endDate = moment$1(task.endDate).format(_this.FORMAT_DATE);
            }
            return task;
        });
        return instances;
...
var AttachFileWidgetComponent = /** @class */ (function (_super) {
    ...
    function () {
        ...
        this.activitiContentService.getAlfrescoRepositories(null, true).subscribe(function (repoList) {
            _this.repositoryList = repoList;
        });
        this.formService.taskSaved.subscribe(function (formSaved) {
    ...
    function (file) {
        ...
        if (this.isTemporaryFile(file)) {
            ...
            this.processContentService.getFileRawContent(((/** @type {?} */ (file))).id).subscribe(function (blob) {
    ...
    function (repository) {
        ...
        if (chosenRepositoryHost !== currentECMHost) {
            ...
            this.attachDialogService.openLogin(formattedRepositoryHost).subscribe(function (selections) {
                var _a;
            ...
            this.contentDialog.openFileBrowseDialogBySite().subscribe(function (selections) {
                var _a;
                (_a = _this.tempFilesList).push.apply(_a, __spread(selections));
                _this.uploadFileFromCS(selections, accountIdentifier);
            });
    ...
    function (fileNodeList, accountId, siteId) {
        ...
        /** @type {?} */
        var filesSaved = [];
        from(fileNodeList).pipe(mergeMap(function (node) {
...
var TaskFilterService = /** @class */ (function () {
    ...
    function (filterId, appId) {
        var _this = this;
        return from(this.callApiTaskFilters(appId)).pipe(map(function (response) { return response.data.find(function (filter$$1) { return filter$$1.id === filterId; }); }), catchError(function (err) { return _this.handleError(err); }));
    };
    /**
    ...
    function (taskName, appId) {
        var _this = this;
        return from(this.callApiTaskFilters(appId)).pipe(map(function (response) { return response.data.find(function (filter$$1) { return filter$$1.name === taskName; }); }), catchError(function (err) { return _this.handleError(err); }));
    };
    /**
...
var TaskFiltersComponent = /** @class */ (function () {
    ...
    function (newFilter) {
        if (newFilter) {
            this.currentFilter = this.filters.find(function (filter$$1, index) {
    ...
    function (taskId) {
        ...
        this.taskListService.getFilterForTaskById(taskId, this.filters).subscribe(function (filter$$1) {
    ...
    function (apps) {
        ...
        if (this.filtersAppId) {
            apps.filter(function (app) {
                _this.filtersAppId.forEach(function (filter$$1) {
