...
import { __assign, __spread, __awaiter, __generator, __extends, __values, __read } from 'tslib';
import { from, throwError, Subject, Observable, of, BehaviorSubject, forkJoin, merge, fromEvent, defer } from 'rxjs';
import { FlatTreeControl } from '@angular/cdk/tree';
import { AlfrescoApiService, AuthenticationService, ContentService, LogService, ThumbnailService, NodesApiService, CoreModule, TranslationService, ObjectDataTableAdapter, SearchService, FileUploadStatus, UploadService, FileModel, FileUtils, DataSorting, ObjectUtils, AllowableOperationsEnum, CardViewUpdateService, MOMENT_DATE_FORMATS, UserPreferenceValues, MomentDateAdapter, UserPreferencesService, SitesService, AppConfigService, TRANSLATION_PROVIDER, CardViewTextItemModel, CardViewDateItemModel, FileSizePipe, CardViewIntItemModel, CardViewFloatItemModel, CardViewDatetimeItemModel, CardViewBoolItemModel, SharedLinksApiService, RenditionsService, HighlightDirective, InfinitePaginationComponent, SearchConfigurationService, EXTENDIBLE_COMPONENT, NotificationService, DisplayMode, ObjectDataColumn, DataColumnListComponent, CustomLoadingContentTemplateDirective, CustomNoPermissionTemplateDirective, CustomEmptyContentTemplateDirective, BaseEvent, NodeDownloadDirective } from '@alfresco/adf-core';
import { NgModule, Injectable, Directive, ElementRef, Renderer2, HostListener, Input, Component, ViewEncapsulation, Inject, EventEmitter, Output, ViewChild, NgZone, Optional, Pipe, ChangeDetectionStrategy, defineInjectable, inject, ComponentFactoryResolver, ViewContainerRef, forwardRef, ChangeDetectorRef, ContentChild, TemplateRef, ViewChildren } from '@angular/core';
import { NodeBodyLock, RatingBody, TagBody, SearchRequest, SiteMemberPaging, Pagination, RequestSortDefinitionInner, SiteEntry, NodeEntry } from '@alfresco/js-api';
import { catchError, switchMap, map, debounceTime, filter, takeUntil, takeWhile, mergeMap, tap, skip } from 'rxjs/operators';
...
var DocumentListService = /** @class */ (function () {
    ...
    /** @nocollapse */ DocumentListService.ngInjectableDef = defineInjectable({ factory: function DocumentListService_Factory() { return new DocumentListService(inject(AuthenticationService), inject(ContentService), inject(AlfrescoApiService), inject(LogService), inject(ThumbnailService)); }, token: DocumentListService, providedIn: "root" });
...
var ContentNodeDialogService = /** @class */ (function () {
    ...
    /** @nocollapse */ ContentNodeDialogService.ngInjectableDef = defineInjectable({ factory: function ContentNodeDialogService_Factory() { return new ContentNodeDialogService(inject(MatDialog$1), inject(ContentService), inject(DocumentListService), inject(SitesService), inject(TranslationService)); }, token: ContentNodeDialogService, providedIn: "root" });
...
var PropertyGroupTranslatorService = /** @class */ (function () {
    ...
    /** @nocollapse */ PropertyGroupTranslatorService.ngInjectableDef = defineInjectable({ factory: function PropertyGroupTranslatorService_Factory() { return new PropertyGroupTranslatorService(inject(LogService)); }, token: PropertyGroupTranslatorService, providedIn: "root" });
...
var TreeViewService = /** @class */ (function () {
    ...
    function (nodeId) {
        return this.nodeApi.getNodeChildren(nodeId)
            .pipe(map(function (nodePage) {
            return nodePage.list.entries.filter(function (node) { return node.entry.isFolder ? node : null; });
        }), map(function (nodes) { return nodes.map(function (node) { return new TreeBaseNode(node); }); }));
    };
